...
The `Napi::Error` class is a persistent reference to a JavaScript error object thus inherits its behavior from the `Napi::ObjectReference` class \(for more info see: [`Napi::ObjectReference`](object_reference.md)\).
