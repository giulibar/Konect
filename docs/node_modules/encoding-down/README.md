...
## Usage with [`level`](https://github.com/Level/level)

The [`level`](https://github.com/Level/level) module conveniently bundles `encoding-down` and passes its `options` to `encoding-down`. This means you can simply do:
...
Please refer to [`level-codec` documentation](https://github.com/Level/codec#encoding-format) for a precise description of the format. Here's a quick example with `level` and `async/await` just for fun:
